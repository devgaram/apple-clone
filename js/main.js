(() => {
  ...
  let currentScene = 0; // 현재 활성화된(눈 앞에 보고 있는) 화면 인덱스
  ...
  const sceneInfo = [
    ...
    },
    {
      ...
      values: {
        ...
        messageA_translateY_in: [20, 0, { start: 0.15, end: 0.3 }],
        messageB_translateY_in: [30, 0, { start: 0.55, end: 0.65 }],
        messageC_translateY_in: [30, 0, { start: 0.9, end: 0.98 }],
        messageA_opacity_in: [0, 1, { start: 0.15, end: 0.3 }],
        messageB_opacity_in: [0, 1, { start: 0.55, end: 0.65 }],
        messageC_opacity_in: [0, 1, { start: 0.9, end: 0.98 }],
        messageA_translateY_out: [0, -20, { start: 0.3, end: 0.35 }],
        messageB_translateY_out: [0, -20, { start: 0.7, end: 0.8 }],
        messageC_translateY_out: [0, -20, { start: 0.98, end: 1 }],
        messageA_opacity_out: [1, 0, { start: 0.3, end: 0.35 }],
        messageB_opacity_out: [1, 0, { start: 0.7, end: 0.8 }],
        messageC_opacity_out: [1, 0, { start: 0.98, end: 1 }],
  ...
  function checkMenu() {
    // window.pageYOffset임, 현재 문서 전체에서 스크롤된 위치
    if (yOffset > 44) {
  ...
  function playAnimation() {
    ...
    switch (currentScene) {
      ...
      case 3:
        let step = 0;
        // 가로 세로 모두 꽉 차게 하기 위해 여기세 세팅(계산 필요)
        const widthRatio = window.innerWidth / objs.canvas.width;
        const heightRatio = window.innerHeight / objs.canvas.height;
        ...
        if (!values.rectStartY) {
          // 스크롤 속도에 따라 이 top 값이 달라지므로 getBoundingClientRect 말고 다른 방법을 써야함
          // values.rectStartY = objs.canvas.getBoundingClientRect().top;
          // 부모도 position:relative로 해야 스크롤 섹션 기준으로 캔버스의 top을 구할 수 있음
          // offsettop값은 transform 적용 전 위치의 값임 canvas는 scale이 적용되어있는 상태라
          // 잘 계산해야함.
          values.rectStartY =
        ...
        if (
          scrollRatio > values.blendHeight[2].end &&
        ...
        if (
          ...
          objs.canvas.classList.remove("sticky");
          objs.canvas.style.marginTop = `${scrollHeight * 0.4}px`; // 위에서 0.2동안 + 0.2동안 애니메이션 했으니 0.4임
  ...
  window.addEventListener("load", () => {
    ...
    // 중간에 로드했을 때, 빈화면 보이다가 스크롤해야 보이ㅣ는 문제 해결
    ...
    // scene3 resize 조절 까다로우니 아예 reload하자
    // window.addEventListener("orientationchange", () => {
    //   scrollTo(0, 0);
    //   setTimeout(() => {
    //     window.location.reload();
  ...
  }); // 리소스(이미지) 다 로드한 후에 레이아웃 계산 하도록 하기위함
